...
class DataLoader_RegressionToy():
    ...
    def get_test_data(self):
        ...
        test_xs = np.expand_dims(np.linspace(-2, 2, num=200, dtype=np.float32), -1)

        test_ys = 5*(test_xs**3) + np.random.normal(scale=0.1, size=test_xs.shape)
...
class DataLoader_RegressionToy_withKink():
    ...
    def __init__(self, args):
        ...
        for i, t in enumerate(self.xs):
            if t > 0.25 or t < -0.25:
                self.ys[i] = 10*(t)**3 + np.random.normal(scale=0.01)
            else:
                self.ys[i] = 30*np.sin(t) + np.random.normal(scale=0.01)
    ...
    def get_test_data(self):
        ...
        test_xs = np.expand_dims(np.linspace(-2, 2, num=1000, dtype=np.float32), -1)

        test_ys = np.zeros(shape=test_xs.shape)
        for i, t in enumerate(test_xs):
            if t > 0.25 or t < -0.25:
                test_ys[i] = 10*(t)**3 + np.random.normal(scale=0.01)
            else:
                test_ys[i] = 30*np.sin(t) + np.random.normal(scale=0.01)
...
class DataLoader_RegressionToy_sinusoidal():
    ...
    def __init__(self, args, infer=False):
        ...
        if not infer:
            self.xs = np.expand_dims(np.linspace(-4, 4, num=1000, dtype=np.float32), -1)
        else:
            self.xs = np.expand_dims(np.linspace(-8, 8, num=2000, dtype=np.float32), -1)

        self.ys = 5*(np.sin(self.xs)) + np.random.normal(scale=0.01, size=self.xs.shape)
    ...
    def get_test_data(self):
        ...
        test_ys = 5*(np.sin(test_xs)) + np.random.normal(scale=0.01, size=test_xs.shape)
